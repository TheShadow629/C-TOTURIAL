...
public:
	...
	void clear()
	{
		if (!m_RootNode)
		{
			...
			while (!q.empty())
			{
				TNode* f = q.front();
				if (f->Left != NULL) 
					q.push(f->Left);//如果子女不为空就加入子女，一会儿删除
				if (f->Right != NULL) 
					q.push(f->Right);
				delete f;//删除当前节点
				q.pop();//把没用的指针弹出
	...
	void display_BinTree_PreOrder(TNode* BT) {//先序遍历打印二叉树
		if (BT == NULL) {
			return;
		}
		else {
			std::cout<< BT->GetData();
			display_BinTree_PreOrder(BT->Left);
			display_BinTree_PreOrder(BT->Right);
		}
	}

	void display_BinTree_InOrder(TNode* BT) {//中序遍历打印二叉树
		if (BT == NULL) {
			return;
		}
		else {
			display_BinTree_InOrder(BT->Left);
			std::cout << BT->GetData();
			display_BinTree_InOrder(BT->Right);
		}
	}

	void display_BinTree_PostOrder(TNode* BT) {//后序遍历打印二叉树
		if (BT == NULL) {
			return;
		}
		else {
			display_BinTree_PostOrder(BT->Left);
			display_BinTree_PostOrder(BT->Right);
			std::cout << BT->GetData();
		}
	}
